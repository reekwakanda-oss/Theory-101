// Storage. node:sqlite ships with Node 22+, so the whole server stays
// dependency-free - nothing from npm to audit, pin, or get compromised.
//
...
db.exec(`
  ...
  -- skipped once every day. Keeping day, used and last_at together, and zeroing
  -- the count when the day changes, avoids that and bounds the table by user.
...
export function sessionUser(tokenHash) {
...
export function assistantUsage(userId) {
  return selectUsage.get(userId) ?? null;
}

export function setAssistantUsage(userId, day, used, lastAt) {
  upsertUsage.run(userId, day, used, lastAt);
}
...
export function purgeExpired(nonceTtlMs) {
